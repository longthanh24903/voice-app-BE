...
if (proxyRotator.getProxyCount() === 0) {
  ...
  console.warn(`[Server] Please add proxies to ${PROXY_FILE} in format: ip:port:user:pass or http://user:pass@ip:port`);
}
console.log(`[Server] Forward secret: ${FORWARD_SECRET ? 'Configured' : 'Not configured (public access allowed)'}`);
...
async function makeProxyRequest(url, method, headers, body) {
  ...
  if (proxyRotator.getProxyCount() === 0) {
    throw new Error('No proxies configured. Please add proxies to proxies.txt file');
  }
  ...
  while (true) {
    ...
    if (attempt > maxAttempts) {
      console.error(`[ProxyRequest] Max attempts (${maxAttempts}) reached. Stopping retry.`);
      throw new Error(`Request failed after ${maxAttempts} attempts. Last error: ${lastError?.message}`);
    }
    ...
    if (!selected || !selected.proxy) {
      ...
      if (remaining === 0 && failed < totalProxies) {
        ...
        console.log(`[ProxyRequest] All proxies used today. Waiting for reset...`);
    ...
    console.log(`[ProxyRequest] Attempt ${attempt} - Using proxy: ${proxyString} (${remaining} remaining in 2-day period, ${daysRemaining} day(s) left)`);
    ...
    try {
      ...
      const isBinaryRequest = url.includes('/text-to-speech/') || 
      ...
      console.log(`[ProxyRequest] Success - Proxy: ${proxyString}, Status: ${response.status}, Attempt: ${attempt}`);
      ...
      console.error(`[ProxyRequest] Failed - Proxy: ${proxyString}, Error: ${error.message}`);
      console.log(`[ProxyRequest] Retrying with different proxy... (${attempt}/${maxAttempts} attempts)`);
...
app.post('/api/proxy-request', checkSecret, async (req, res) => {
  try {
    ...
    } else if (contentType.includes('application/json') || typeof result.body === 'object') {
      // JSON response
...
app.listen(PORT, () => {
  ...
  console.log(`[Server] Proxy endpoint: http://localhost:${PORT}/api/proxy-request`);
});
